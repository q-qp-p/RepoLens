...
# Greenfield — plan from a split specification
./repolens.sh --project ~/my-app --agent claude --mode greenfield \
  --spec-dir ~/docs/product-spec --spec-entry index.md \
  --spec-exclude 'notes/**'
...
The complete patch is written once to `logs/<run-id>/branch-diff.txt`, and a bounded manifest — the base and head refs, the base, head, and merge-base commits, the changed-file list, and the diffstat — to `logs/<run-id>/branch-manifest.md`. Only the manifest is placed in lens prompts, so a large branch does not inflate every prompt; lenses read per-file patches on demand from the recorded commits. `--dry-run` prints all three commits (base, head, merge base) plus both artifact paths before anything runs.
...
Resuming a branch review rehydrates the base and head refs and the three commits from the persisted manifest instead of recomputing them, so the delta and displayed provenance stay pinned to the run's original review head. A non-empty resume proceeds only when that persisted head is still checked out; otherwise RepoLens stops before dispatching agents. Check out the persisted head recorded in `logs/<run-id>/branch-manifest.md` to continue the frozen run, or start a new branch review at the current head. `--branch-base` and `--branch-head` are therefore optional on resume; for a non-empty manifest, supplying either flag requires an exact match with its persisted ref spelling. Two ref names that resolve to the same commit still conflict when their spellings differ.
...
| `--branch-base <ref>`  | Base ref the review head is compared against. Required for `--mode branch-review` (except on `--resume`, which reads it back from the run's manifest) and rejected in every other mode. When supplied on a non-empty resume, its spelling must match the persisted base ref. The delta is three-dot — everything the head added on top of the merge base it shares with this ref — so commits that exist only on the base are never reported as branch changes. A ref that does not resolve locally stops the run before any agent starts (fetch it first if it only exists on the remote). |
| `--branch-head <ref>`  | Review head for `--mode branch-review` (default: `HEAD`). Must resolve to the checked-out commit, because lenses read the working tree — reviewing a head that is not checked out would pair a diff from one commit with source from another. On a non-empty resume, omitting this flag restores the persisted head ref for provenance; supplying it requires an exact spelling match with the persisted ref. Rejected outside `branch-review`. |
...
3. If `--dry-run`: prints mode, agent, project path, resolved round count, remote target metadata when `--remote` is set, multi-file specification selection details when `--spec-dir` is set, and the full lens list, then exits — no agents run, no prompts are shown, and no Android Gradle build is executed
...
9. For each lens:
   ...
   - Agent follows the selected mode: code modes read code, deploy modes inspect the target, content modes inspect content/source material, greenfield plans from a single-file or directory specification using the current open issue or local draft backlog without inspecting repository code, and polish mode looks for voice-fit refinements
...
- **Branch Review Artifacts** — `branch-review` runs write the complete branch patch to `logs/<run-id>/branch-diff.txt` and a manifest to `logs/<run-id>/branch-manifest.md` recording the base and head refs, the base, head, and merge-base commits, the changed-file count and name-status list, and the diffstat. An empty delta writes both files empty. Resume reads both refs and the commits back from the manifest
